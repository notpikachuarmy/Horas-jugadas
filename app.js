...
function csvUrl(url){
  // El enlace de Series facilitado es pubhtml; lo convertimos al endpoint CSV de Google Sheets.
...
async function loadCSV(url){
  const r=await fetch(csvUrl(url),{cache:"no-store"});
...
(async()=>{
 try{
  [games,series,streams]=await Promise.all([loadCSV(DATA.games),loadCSV(DATA.series),loadCSV(DATA.streams)]);
  renderGames();renderSeries();renderStats();
 }catch(e){
  console.error(e);
  document.getElementById("gamesList").innerHTML=`<div class="empty">No se han podido cargar los datos. Comprueba que las tres pestañas de Google Sheets están publicadas en la web y que sus enlaces son accesibles.<br><small>${esc(e.message||e)}</small></div>`;
 }
})();
